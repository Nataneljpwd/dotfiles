...
# 2. Get Application Names (extracted from .desktop files)
APPS=$(grep -rh "^Name=" $APP_DIRS 2>/dev/null | cut -d= -f2- | grep -vE "nvim|java|server|gcc|clang" | sort -u)
...
# 4. Use printf to ensure a clean vertical list for bemenu
SELECTED=$(printf "%s\n%s\n%s" "$PROFILES" "$UTILS" "$APPS" | bemenu $BEMENU_OPTS -p "Launcher:")

case "$SELECTED" in
    "network manager") exec $HOME/scripts/net_menu.sh ;;
    "bluetooth manager (bt)") exec $HOME/scripts/bt_menu.sh ;;
    "study")
        SUBJECT=$(ls -1 $HOME/learning | bemenu $BEMENU_OPTS -p "Course:")
        [ -z "$SUBJECT" ] && exit
        swaymsg "workspace 2; exec rnote $HOME/learning/$SUBJECT"
        swaymsg "workspace 3; exec firefox -P '$SUBJECT'"
        ;;
    "development")
        PROJECT=$(ls -1 $HOME/projects | bemenu $BEMENU_OPTS -p "Project:")
        [ -z "$PROJECT" ] && exit
